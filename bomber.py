...
colorama.init()
root = Tk()
root.title('СМС Бомбер, by DSD-dev')
    ...
    if var.get() == 1:
        ...
        try:
            if len(number) < 12 or file_name == None:
                ...
                messagebox.showinfo(title='G00D', message='Спам запущен')
...
crack = Button(text='БОМБИТЬ', height=2, width=12, background='red', command=StartThread)
crack.pack()
